...
{
    internal class RateScore : ISubmissionRater
    {
        public RatingMethod Name => RatingMethod.Score;

        public int Compare(SubmissionEntry x, SubmissionEntry y)
        {
            return x.CustomScore - y.CustomScore;
        }
        ...
        public void UpdatePoints(List<SubmissionEntry> submitters, RatingPoints points)
        {
            ...
            {
                submitters[0].Points = pointsForBest33;
                if (submitters[0].CustomScore == submitters[1].CustomScore)
                {
            ...
            foreach (var submitter in submitters)
            {
                ...
                {
                    if (submitters[(int) index - 2].CustomScore == submitter.CustomScore && submitters[(int) index - 2].Points == pointsForBest33)
                    {
                        submitter.Points = pointsForBest33;
                    }
                    else
                ...
                else
                {
                    ...
                    else if (submitters[(int) index - 2].CustomScore == submitter.CustomScore && submitters[(int) index - 2].Points == pointsForBest66)
                    {
                        submitter.Points = pointsForBest66;
                    }
                    else
        ...
        public void UpdateRanking(List<SubmissionEntry> submitters)
        {
            foreach (var submitter in submitters)
            {
                var rank = 1 + submitters.Count(x => x.CustomScore > submitter.CustomScore);
                submitter.Rank = rank;
            }
        }
    }
