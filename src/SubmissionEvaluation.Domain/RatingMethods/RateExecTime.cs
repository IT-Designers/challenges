...
{
    internal class RateExecTime : ISubmissionRater
    {
        public RatingMethod Name => RatingMethod.Exec_Time;

        public int Compare(SubmissionEntry x, SubmissionEntry y)
        {
            return y.Exectime - x.Exectime;
        }
        ...
        public void UpdatePoints(List<SubmissionEntry> submitters, RatingPoints points)
        {
            ...
            {
                submitters[0].Points = pointsForBest33;
                if (submitters[0].Exectime == submitters[1].Exectime)
                {
            ...
            foreach (var submitter in submitters)
            {
                ...
                {
                    if (submitters[(int) index - 2].Exectime == submitter.Exectime && submitters[(int) index - 2].Points == pointsForBest33)
                    {
                        submitter.Points = pointsForBest33;
                    }
                    else
                ...
                else
                {
                    ...
                    else if (submitters[(int) index - 2].Exectime == submitter.Exectime && submitters[(int) index - 2].Points == pointsForBest66)
                    {
                        submitter.Points = pointsForBest66;
                    }
                    else
        ...
        public void UpdateRanking(List<SubmissionEntry> submitters)
        {
            foreach (var submitter in submitters)
            {
                var rank = 1 + submitters.Count(x => x.Exectime < submitter.Exectime);
                submitter.Rank = rank;
            }
        }
    }
