...
{
    public class CppCMakeCompiler : CompilerBase
    {
        ...
        public CppCMakeCompiler(ILog log, string compilerPath, string pathToCppCheck, string pathToCMake)
        {
            this.log = log;
...
";
        ...
        public override string LatexCodeExtension => ".cpp";
        ...
        {
            return files.Any(x => x.EndsWith(".cpp", StringComparison.CurrentCultureIgnoreCase));
        }

        public override IEnumerable<string> GetSourceFiles(IEnumerable<string> files)
        {
            return files.Where(x => (x.EndsWith(".cpp", StringComparison.CurrentCultureIgnoreCase) ||
                                     x.EndsWith(".h", StringComparison.CurrentCultureIgnoreCase) ||
                                     x.EndsWith(".hpp", StringComparison.CurrentCultureIgnoreCase) ||
                                     x.Equals("CMakeLists.txt", StringComparison.CurrentCultureIgnoreCase) ||
                                     x.EndsWith(".h++", StringComparison.CurrentCultureIgnoreCase)) && !x.EndsWith("FixBufferIssue_Generated.cpp"));
        }
        ...
        {
            const string fixBufferIssueFileName = "FixBufferIssue_Generated.cpp";
            var compilableFiles = Directory.GetFiles(paths.Host.SourcePath, "*.cpp", SearchOption.AllDirectories);
        ...
        private CMakeList LoadCMakelist(CompilerPaths paths, IEnumerable<string> compilableFiles)
        {
            ...
            {
                cmakeList = CMakeList.GetDefaultCMakeListForCpp(files.ToArray());
            }
