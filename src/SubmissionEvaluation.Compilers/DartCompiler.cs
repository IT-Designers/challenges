...
{
    public class DartCompiler : CompilerBase
    {
        private readonly string compilerPath;
        private readonly ILog log;

        public DartCompiler(ILog log, string compilerPath)
        {
            this.log = log;
        ...
        public override string LatexCodeExtension => ".dart";
        ...
        {
            return files.Where(x => x.EndsWith(".dart", StringComparison.CurrentCultureIgnoreCase));
        }
        ...
        public override ExecutionParameters CompileContent(CompilerPaths paths, IProcessProvider processProvider, bool forceRecompile)
        {
            ...
            var compilableFiles = Directory.GetFiles(paths.Host.SourcePath, "*.dart", SearchOption.AllDirectories);
            log.Information("{count} dart Dateien zum kompelieren gefunden.", compilableFiles.Length);
            ...
            else
            {
                ...
                try
                {
                    ...
                    var result = processProvider.Execute(compilerPath, new[] {"compile", "exe", "-o", pathToBinary}.Concat(files).ToArray(),
